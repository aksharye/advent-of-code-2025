...
int main() {
    ...
    // perform DP to calculate paths starting from svr
    dp["svr"] = 1;
    for (string x : topo) {
        for (string y : rev[x]) dp[x] += dp[y];
    }

    // number of paths from svr->dac, svr->fft
    ll D = dp["dac"];
    ll F = dp["fft"];
    dp.clear();

    // perform DP to calculate paths starting from dac
    dp["dac"] = 1;
    for (string x : topo) {
        for (string y : rev[x]) dp[x] += dp[y];
    }

    // number of paths from dac->fft, dac->out
    ll DF = dp["fft"];
    ll DO = dp["out"];
    dp.clear();

    // perform DP to calculate paths starting from fft
    dp["fft"] = 1;
    for (string x : topo) {
        for (string y : rev[x]) dp[x] += dp[y];
    }

    // number of paths from fft->dac, fft->out
    ll FD = dp["dac"];
    ll FO = dp["out"];

    // total paths is:
    // #(svr->dac) * #(dac->fft) * #(fft->out) and
    // #(svr->fft) * #(fft->dac) * #(dac->out)
    ll paths = D * DF * FO + F * FD * DO;
